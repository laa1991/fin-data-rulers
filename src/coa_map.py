...
def col_for_period(table: dict, period: str) -> int:
    ...
    ⚠⚠ **必须在算索引前把「附注」列从表头里去掉**：`vals` 是**剔完附注列之后**的，
    而表头还是原始的 —— 两边口径不一致 ⇒ 索引差一格 ⇒ **读到上一年的数**。
    实测：格力 2023 总资产真值 368.05e9，而表里取到 355.07e9（= 2022 的值），量级正常、不报警。
    """
    year = re.match(r"(\d{4})", period or "")
    year = year.group(1) if year else None
    hdr = [h for h in (table.get("header") or [])[1:]
           if nrm(str(h)) != "附注"]                     # ← 与 vals 的口径对齐
    # ⚠⚠⚠ **只有表头真的像表头时才信它**：实测格力的表头是**页眉文字**
    #   `['', '珠海', '格力电器股份有限公司2023年年度', '报告全文', '']` —— 「2023年」出现在**公司名里**，
    #   于是「按年份认列」算出一个**毫无意义的下标 1** ⇒ 总资产取到 3550.71 亿（=2022），真值 3680.54 亿。
    #   判据：**值列里至少要有 2 格带日期**才算真表头（真报表是「项目 | 2023年12月31日 | 2022年12月31日」）。
    date_cells = sum(1 for h in hdr if re.search(r"\d{4}\s*年", str(h)))
    if year and date_cells >= 2:
