...
HEAD = [("合并资产负债表", "BS", "合并"), ("合并利润表", "IS", "合并"), ("合并现金流量表", "CF", "合并"),
        # ⚠ 「母公司+表名」必须**显式列出**：`is_heading` 要求标题在**行首**，
        #   而裸名（「现金流量表」）匹配不上「母公司现金流量表」这种行首 ⇒ 整页被判成合并。
        #   实测代价：伊利 p93 = 母公司现金流量表 被标成合并，于是那一格取了母公司的事（8.43e9）。
        ("母公司资产负债表", "BS", "母公司"), ("母公司利润表", "IS", "母公司"),
        ("母公司现金流量表", "CF", "母公司"), ("本公司资产负债表", "BS", "母公司"),
        ("本公司利润表", "IS", "母公司"), ("本公司现金流量表", "CF", "母公司"),
...
# ⚠⚠ 冒号**不能要求**：中国平安写的是「除特别注明外，金额单位**为**人民币百万元」（无冒号）——
#   旧正则 `单位[:：]` 对它静默失配 ⇒ 那一家的所有数**量级差 100 万倍**而状态照样 ✓。
UNIT = re.compile(r"单位\s*[:：为是]?\s*(?:人民币)?\s*(亿元|百万元|千万元|万元|千元|元)")
...
def align_period(rows, header, year):
    """把一页的行归一到**本期那一列**：按本页表头找年份所在的列，只留该列。

    为什么需要它：同一张表的多页**各自解析**，列的含义可以不一致（实测伊利 CF：p91 取到 2022 的列、
    p92 取到 2023 的列，两个数量级都正常）。归一之后再拼，才既不串列也不丢行。
    """
    k = 0
    for j, c in enumerate(header or []):
        if year and year in str(c):
            k = max(0, j - 1)          # header 第 0 格是「项目/标签」列，故减 1
            break
    ...
    for it in seq:
        ...
        if run and it["page"] == run[-1]["pages"][-1] + 1 and len(run[-1]["pages"]) < MAX_RUN_PAGES:
            ...
            if w_it and w_run and w_it != w_run:
                # **拒拼**（宁缺勿错）。试过升级成「按各自表头归一到本期列再拼」，**失败并回退**：
                #   前提是「表头格数 = 行宽 + 1」，而表头可能来自上一页或表头修复路径 ⇒ 索引不可信，
                #   实测伊利 CF 归一后「经营活动净额」变成 8,434,869,082.25（既非 2023 的 18.29e9、
                #   也非 2022 的 13.42e9），且出现两行同名 —— **会产出错值的版本比缺值更坏**。
                #   正解（下次）：先按**内容**剔附注列（全为小整数/空）让宽度一致，再谈归一。
